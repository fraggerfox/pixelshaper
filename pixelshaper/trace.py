...
def trace(project: Project, force: bool = False, log=print) -> dict:
    ...
    if ppem is None:
        ppem = suggested
        log(f"ppem not pinned; suggesting {ppem} "
    ...
    for gid in sorted(gids):
        ...
        art = glyphart.from_bitmap(name, advance, left, top,
                                   gray_rows, project.threshold)
    ...
    log(f"{len(gids)} glyphs needed: {len(written)} traced, "
        f"{len(kept)} kept (hand-edits preserved)")
    return {"ppem": ppem, "needed": len(gids),
